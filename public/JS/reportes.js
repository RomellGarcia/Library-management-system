...
// Convierte tasa continua k a porcentaje mensual: (e^k - 1) * 100
...
// k, C y t0 siempre vienen de la API — fallbacks seguros
...
function renderResumen() {
  ...
  document.getElementById('categoriasCrecimiento').innerHTML = catsConTasa
    .filter(function(c){ return c.k > 0; }).slice(0,5)
    .map(function(c){
  ...
  document.getElementById('categoriasDecrecimiento').innerHTML = catsConTasa
    .filter(function(c){ return c.k < 0; }).slice(0,5)
    .map(function(c){
      var ultimo = c.prestamos[c.prestamos.length-1];
      return '<li><span class="reco-titulo">'+c.nombre+'<small>'+ultimo+' prestamos el ultimo mes</small></span>'+
...
function renderLibros() {
  ...
  var librosConTasa = DATA.libros.map(function(l) {
    ...
    var proy     = C > 0 ? redondear(proyectar(C, k, tFinal+1, t0)) : actual;
    return {
  ...
  document.getElementById('librosCrecimiento').innerHTML = librosConTasa
    .filter(function(l){ return l.k > 0; }).slice(0,6)
    .map(function(l){
  ...
  document.getElementById('librosDecrecimiento').innerHTML = librosConTasa
    .filter(function(l){ return l.k < 0; }).slice(0,6)
    .map(function(l){
      return '<li><span class="reco-titulo">'+l.nombre+'<small>'+l.categoria+' - '+l.actual+' prestamos el ultimo mes</small></span>'+
  ...
  document.getElementById('tablaLibros').innerHTML = librosConTasa.map(function(libro) {
    ...
    var colorProy = libro.k >= 0 ? '#2E7D32' : '#E65100';
    return '<tr>'+
      ...
      '<td style="text-align:center;color:'+colorProy+';font-weight:600">'+libro.proyeccion+'</td>'+
      ...
      datasets: [
        {
          ...
          borderColor: '#A02142', backgroundColor: colorAlpha('#A02142',0.15),
          borderWidth: 2, pointBackgroundColor: '#A02142'
        },
        {
...
function calcularProyeccion() {
  ...
  // Tabla del modelo — muestra datos históricos + parámetros calculados por regresión
  if (document.getElementById('tablaModelo')) {
    var filasHistorico = prestamos.map(function(x, t) {
      ...
      var lnX      = x > 0 ? Math.log(x).toFixed(4) : '—';
      return '<tr>'+
        '<td>'+t+'</td>'+
        '<td>'+mesLabel+'</td>'+
        '<td>'+x+'</td>'+
    ...
    document.getElementById('tablaModelo').innerHTML =
      '<p style="font-weight:600;margin-bottom:6px">Datos históricos (ln(x) para regresión lineal):</p>'+
      '<table class="tabla-modelo" style="margin-bottom:14px">'+
      '<thead><tr><th>t</th><th>Mes</th><th>Préstamos x(t)</th><th>ln(x)</th></tr></thead>'+
      '<tbody>'+filasHistorico+'</tbody>'+
      '</table>'+
      '<p style="font-weight:600;margin-bottom:6px">Parámetros del modelo (regresión lineal exponencial):</p>'+
      '<table class="tabla-modelo" style="margin-bottom:12px">'+
      '<thead><tr><th>Parámetro</th><th>Valor</th><th>Significado</th></tr></thead>'+
      '<tbody>'+
      '<tr><td><strong>C</strong></td><td>'+C.toFixed(4)+'</td><td>Valor inicial ajustado (e^intercepto)</td></tr>'+
      '<tr><td><strong>k</strong></td><td>'+k.toFixed(4)+'</td><td>Tasa de crecimiento continua (pendiente)</td></tr>'+
      '<tr><td><strong>t0</strong></td><td>'+t0+'</td><td>Tiempo de referencia</td></tr>'+
      '</tbody></table>'+
      '<p style="font-size:0.87rem;color:#555">Fórmula aplicada: x(t) = '+C.toFixed(4)+' · e^('+k.toFixed(4)+' · t)</p>'+
      '<p style="font-size:0.87rem;color:#555">Proyección mes siguiente: x('+(tFinal+1)+') = '+C.toFixed(4)+' · e^('+k.toFixed(4)+' · '+(tFinal+1)+') = <strong>'+proyecciones[0]+'</strong></p>';
  }
  ...
  // Gráfica: línea real + línea de proyección empalmadas
