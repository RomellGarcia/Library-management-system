...
// ====================== UTILIDADES ======================
// Modelo: x(t) = C · e^(k · t)
// C y k se calculan en el backend con el método adecuado:
//   - Punto promedio  → si todos los meses tienen préstamos
//   - Mínimos cuadrados → si algún mes tiene 0 préstamos
...
// Etiqueta del método usado (para mostrar en la tabla del modelo)
function etiquetaMetodo(metodo) {
  if (metodo === 'promedio')          return 'Punto Promedio';
  if (metodo === 'minimos_cuadrados') return 'Mínimos Cuadrados';
  return 'Sin calcular';
}
...
function cargarDatos() {
  return Promise.all([
    ...
    if (!resultados[0].success) throw new Error(resultados[0].message || 'Error al obtener datos');
    DATA  = resultados[0].data;
    STATS = resultados[1].success ? resultados[1].data : null;
    return true;
  });
}
...
function renderResumen() {
  ...
  html += '<div class="stat-card"><div class="stat-label">Libros en el Acervo</div><div class="stat-value">' + totalLibros + '</div><div class="stat-desc">Titulos disponibles para prestamo</div></div>';
  ...
  chartInstances['chartPrestamosMes'] = new Chart(document.getElementById('chartPrestamosMes'), {
    ...
    data: {
      labels: mesesLabel,
      datasets: [{ label: 'Prestamos', data: totalesMes,
  ...
  document.getElementById('categoriasCrecimiento').innerHTML = catsConTasa.filter(function(c) { return c.k > 0; }).slice(0, 5).map(function(c) {
    ...
    return '<li><span class="reco-titulo">' + c.nombre + '<small>' + ultimo + ' prestamos el ultimo mes</small></span><span class="reco-cambio up">+' + tasaAPorcentaje(c.k) + '% / mes</span></li>';
  ...
  document.getElementById('categoriasDecrecimiento').innerHTML = catsConTasa.filter(function(c) { return c.k < 0; }).slice(0, 5).map(function(c) {
    var ultimo = c.prestamos[c.prestamos.length - 1];
    return '<li><span class="reco-titulo">' + c.nombre + '<small>' + ultimo + ' prestamos el ultimo mes</small></span><span class="reco-cambio down">' + tasaAPorcentaje(c.k) + '% / mes</span></li>';
...
function renderCategorias() {
  ...
  chartInstances['chartRadar'] = new Chart(document.getElementById('chartRadar'), {
    ...
    data: { labels: DATA.categorias.map(function(c) { return c.nombre; }),
      datasets: [
        { label: 'Mes actual', data: DATA.categorias.map(function(c) { return c.prestamos[c.prestamos.length - 1] || 0; }), borderColor: '#A02142', backgroundColor: colorAlpha('#A02142', 0.15), borderWidth: 2, pointBackgroundColor: '#A02142' },
        { label: 'Estimacion proximo mes', data: DATA.categorias.map(function(c) {
          var k = obtenerK(c); var C = obtenerC(c); var t0 = obtenerT0(c); var tFinal = c.prestamos.length - 1;
          return c.datos_suficientes ? redondear(proyectar(C, k, tFinal + 1, t0)) : 0;
  ...
  chartInstances['chartKCategorias'] = new Chart(document.getElementById('chartKCategorias'), {
    ...
    data: { labels: catsConTasa.map(function(c) { return c.nombre; }),
      datasets: [{ label: 'Cambio mensual %', data: catsConTasa.map(function(c) { return parseFloat(tasaAPorcentaje(c.k)); }),
...
function calcularProyeccion() {
  ...
  var metodo    = item.metodo || 'ninguno';
  ...
  // ── Sin datos suficientes (menos de 2 meses con préstamos) ──
  if (!item.datos_suficientes) {
    document.getElementById('resultadoTexto').innerHTML =
      ...
      'Se necesitan al menos <strong>2 meses con préstamos</strong> para calcular una tendencia confiable.';
  ...
  // ── Advertencia si tiene menos de 6 meses con datos ──
  var advertencia = mesesConDatos < 6
    ? '<div style="background:#FFF8E1;border-left:4px solid #BC955B;padding:10px 14px;margin-bottom:12px;border-radius:4px;font-size:0.88rem">' +
      '⚠️ Predicción basada en <strong>' + mesesConDatos + ' de 6 meses</strong> con préstamos. ' +
      'La estimación mejorará conforme el libro acumule más historial.</div>'
    : '';
  ...
  if (document.getElementById('tablaModelo')) {
    ...
    if (metodo === 'promedio') {
      // Tabla método punto promedio
      var tProm = item.tPromedio;
      var pProm = item.pPromedio;
      var filasDatos = prestamos.map(function(x, t) {
        return '<tr><td>' + t + '</td><td>' + x + '</td></tr>';
      }).join('');

      contenidoTabla =
        '<p style="font-weight:600;margin-bottom:6px">Método: <span style="color:#2E7D32">Punto Promedio</span> (todos los meses tienen préstamos)</p>' +
        '<table class="tabla-modelo" style="margin-bottom:12px">' +
        '<thead><tr><th>t</th><th>Préstamos (x)</th></tr></thead>' +
        '<tbody>' + filasDatos + '</tbody></table>' +
        '<p style="margin-bottom:6px">Promedios: <strong>t̄ = ' + tProm + '</strong> &nbsp;|&nbsp; <strong>P̄ = ' + pProm + '</strong></p>' +
        '<p style="margin-bottom:12px">Despeje de k: <code>' + pProm + ' = ' + C + ' · e^(k · ' + tProm + ')</code> → <strong>k = ' + k.toFixed(4) + '</strong></p>';

    } else {
      // Tabla método mínimos cuadrados
      var filasLineal = prestamos.map(function(x, t) {
        var xAjustado = x === 0 ? 0.1 : x;
        var lnX = Math.log(xAjustado).toFixed(4);
        return '<tr><td>' + t + '</td><td>' + x + (x === 0 ? ' → 0.1' : '') + '</td>' +
               '<td>' + lnX + '</td><td>' + (t * t) + '</td><td>' + (t * Math.log(xAjustado)).toFixed(4) + '</td></tr>';
      }).join('');

      contenidoTabla =
        '<p style="font-weight:600;margin-bottom:6px">Método: <span style="color:#1565C0">Mínimos Cuadrados</span> (hay meses con 0 préstamos)</p>' +
        '<table class="tabla-modelo" style="margin-bottom:12px">' +
        '<thead><tr><th>t</th><th>x (prest.)</th><th>ln(x)</th><th>t²</th><th>t·ln(x)</th></tr></thead>' +
        '<tbody>' + filasLineal + '</tbody></table>';
    }

    // Tabla resumen CI / K / predicción
    contenidoTabla +=
      ...
      '</tbody></table>' +
      '<p style="margin-top:10px;font-size:0.87rem;color:#666">Fórmula: x(t) = ' + C + ' · e^(' + k.toFixed(4) + ' · t)</p>';

    document.getElementById('tablaModelo').innerHTML = contenidoTabla;
  }
  ...
  chartInstances['chartProyeccion'] = new Chart(document.getElementById('chartProyeccion'), {
    ...
    data: { labels: labelsAll, datasets: [
      { label: 'Prestamos reales', data: datosReal, borderColor: '#A02142', backgroundColor: colorAlpha('#A02142', 0.1), borderWidth: 2.5, fill: true, tension: 0.3, pointRadius: 5, pointBackgroundColor: '#A02142', spanGaps: false },
      { label: 'Proyeccion x(t) = C·e^(k·t)', data: datosProyeccion, borderColor: '#BC955B', backgroundColor: colorAlpha('#BC955B', 0.08), borderWidth: 2.5, borderDash: [8, 4], fill: true, tension: 0.3, pointRadius: 5, pointBackgroundColor: '#BC955B', pointStyle: 'triangle', spanGaps: false }
    ] },
    options: { responsive: true,
      plugins: { legend: { position: 'bottom', labels: { usePointStyle: true, padding: 16 } },
        tooltip: { callbacks: { label: function(ctx) { if (ctx.parsed.y === null) return null; return ctx.dataset.label + ': ' + redondear(ctx.parsed.y) + ' prestamos'; } } } },
      scales: {
        y: { beginAtZero: true, grid: { color: '#E0D8D0' }, ticks: { precision: 0 }, title: { display: true, text: 'x(t)', font: { weight: 600 } } },
        x: { grid: { display: false }, title: { display: true, text: 't (meses desde ' + formatearMes(DATA.meses[0]) + ')', font: { weight: 600 } } }
      } }
  });
}
