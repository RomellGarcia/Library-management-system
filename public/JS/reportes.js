...
// ====================== UTILIDADES ======================
// Modelo: x(t) = C · e^(k · t)   (t0 siempre = 0, primer mes del rango)
// Derivado de la Ley de Crecimiento/Decaimiento:
//   dx/dt = kx  →  dx/x = k dt  →  ln(x) = kt + ln(C)  →  x = Ce^(kt)
// C y k se calculan en el backend con mínimos cuadrados sobre los 6 meses.
...
// k, C y t0 vienen de la API (calculados con mínimos cuadrados sobre 6 meses)
...
function renderLibros() {
  ...
  var librosConTasa = DATA.libros.map(function(l) {
    ...
    var C    = obtenerC(l);
    var t0   = obtenerT0(l);
    var tFinal  = l.prestamos.length - 1; // t=5 (último mes histórico)
    var actual  = l.prestamos[tFinal] || 0;
    var anterior = tFinal >= 1 ? l.prestamos[tFinal - 1] : 0;
    // Proyección al siguiente mes: t = tFinal + 1
    var proy = redondear(proyectar(C, k, tFinal + 1, t0));

    return {
      nombre: l.nombre, categoria: l.categoria || 'Sin categoria',
      anterior: anterior, actual: actual, k: k, proyeccion: proy, prestamos: l.prestamos
    };
  });
...
function renderCategorias() {
  ...
  chartInstances['chartCategoriasLinea'] = new Chart(document.getElementById('chartCategoriasLinea'), {
    ...
    data: {
      ...
      datasets: DATA.categorias.map(function(c, i) {
        ...
        return {
          label: c.nombre, data: c.prestamos, borderColor: color,
  ...
  var catsConTasa = DATA.categorias.map(function(c) {
    ...
    return {
      nombre: c.nombre, total: total, promedio: (total / c.prestamos.length).toFixed(1),
      ...
      datasets: [
        ...
        },
        {
          ...
          data: DATA.categorias.map(function(c) {
            ...
            return redondear(proyectar(C, k, tFinal + 1, t0));
          }),
...
function calcularProyeccion() {
  ...
  var k  = obtenerK(item);   // tasa calculada con mínimos cuadrados sobre 6 meses
  var C  = obtenerC(item);   // condición inicial ajustada = e^b
  var t0 = obtenerT0(item);  // siempre 0
  var tFinal = prestamos.length - 1; // t=5 (último mes histórico, ej: Abr)
  ...
  // Calcular proyecciones para cada mes futuro
  // x(t) = C · e^(k · t)   con t = tFinal+1, tFinal+2, ...
  var proyecciones = [];
  var proyGrafica  = [];
  var mesesFuturos = [];
  ...
  for (var i = 1; i <= periodos; i++) {
    var tAbs       = tFinal + i;                              // t absoluto desde t0=0
    var valorExacto = proyectar(C, k, tAbs, t0);             // C · e^(k · tAbs)
  ...
  // Tabla del modelo matemático: muestra C, k y los pasos del cálculo
  // para que puedas verificar que coinciden con tu libreta
  if (document.getElementById('tablaModelo')) {
    // Construir tabla de los 6 meses históricos con ln(x) para verificación
    var filasHistorico = prestamos.map(function(x, t) {
      var xAjustado = x === 0 ? 0.1 : x;
      var lnX = Math.log(xAjustado).toFixed(4);
      var t2  = t * t;
      var tY  = (t * Math.log(xAjustado)).toFixed(4);
      return '<tr><td>' + t + '</td><td>' + x + (x === 0 ? ' → 0.1' : '') + '</td>' +
             '<td>' + lnX + '</td><td>' + t2 + '</td><td>' + tY + '</td></tr>';
    }).join('');

    document.getElementById('tablaModelo').innerHTML =
      // Tabla de datos históricos
      '<p style="font-weight:600;margin-bottom:6px">Tabla de linealización (ln x = kt + b):</p>' +
      '<table class="tabla-modelo" style="margin-bottom:16px">' +
      '<thead><tr><th>t</th><th>x (prest.)</th><th>ln(x)</th><th>t²</th><th>t·ln(x)</th></tr></thead>' +
      '<tbody>' + filasHistorico + '</tbody>' +
      '</table>' +
      // Resultados del modelo
      '<p style="font-weight:600;margin-bottom:6px">Resultado del modelo:</p>' +
      '<table class="tabla-modelo">' +
      '<thead><tr><th>Variable</th><th>Valor</th><th>Significado</th></tr></thead>' +
      '<tbody>' +
      '<tr><td><strong>C</strong></td><td>' + C + '</td><td>Condicion inicial ajustada (e^b)</td></tr>' +
      '<tr><td><strong>k</strong></td><td>' + k.toFixed(4) + '</td><td>' + (k >= 0 ? 'Crecimiento' : 'Decrecimiento') + ' mensual</td></tr>' +
      '<tr><td><strong>t₀</strong></td><td>' + t0 + '</td><td>Mes base (' + formatearMes(DATA.meses[0]) + ')</td></tr>' +
      '<tr><td><strong>Fórmula</strong></td><td colspan="2">x(t) = ' + C + ' · e^(' + k.toFixed(4) + ' · t)</td></tr>' +
      '</tbody>' +
      '</table>';
  }
  ...
  datosProyeccion.push(x0); // punto de empalme = último valor real
  ...
  chartInstances['chartProyeccion'] = new Chart(document.getElementById('chartProyeccion'), {
    ...
    options: {
      ...
      scales: {
        y: {
          beginAtZero: true, grid: { color: '#E0D8D0' }, ticks: { precision: 0 },
